...
public class SyntyStoreImporter : EditorWindow
{
    ...
    private static readonly string _HEADER_TEXTURE = "Assets/Plugins/Synty/Editor/SyntyStore_Importer/synty_packages_header.png";
    ...
    private void HandlePlayModeStateChange(PlayModeStateChange state)
    {
        ...
        {
            RefreshAndCacheTextures();
        }
    }

    private void RefreshAndCacheTextures()
    {
        RefreshDirectory();
        _infoImg.Clear();
        _textureCache.Clear();
        ...
        foreach (FileInfo packageFile in _info)
        {
            Texture2D texture = FindMatchingImage(packageFile, imageFiles);

            if (texture == null)
            {
                _infoImg.Add(_placeholderImage);
            }
            else
            {
                _infoImg.Add(texture);
            }
        }
    }
    ...
    {
        bool dialogResponse = EditorUtility.DisplayDialog("WARNING!",
    ...
    private void ImportSelected()
    {
        ...
        {
            EditorUtility.DisplayDialog("No Packages Selected",
    ...
    private void RefreshDirectory()
    {
        ...
        _placeholderImage = LoadPNG("Assets/Plugins/Synty/Editor/SyntyStore_Importer/synty_logo_white.png");
